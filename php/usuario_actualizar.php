<?php 
    ...
    if ($admin_usuario=="" || $admin_clave=="") {
        echo '
            <div class="notification is-danger is-light">
                <strong>¡Ocurrio un error inesperado!</strong><br>
                No hallenado los campos que corresponden a su USUARIO y/o CLAVE
            </div>
            ';
            exit();
    }
    ...
    $check_admin = $check_admin -> query("SELECT usuario_usuario FROM usuario WHERE usuario_usuario = '$admin_usuario' AND usuario_id='".$_SESSION['id']."'");
    ...
    if ($check_admin -> rowCount()==1) {
        ...
        $check_admin['usuario_clave'])) {
            ...
            <div class="notification is-danger is-light">
                <strong>¡Ocurrio un error inesperado!</strong><br>
                USUARIO y/o CLAVE de administrador incorrectas
            </div>
            ';
            exit();
        }
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($check_email -> rowCount()>0) {
                echo '
                    <div class="notification is-danger is-light">
                        <strong>¡Ocurrio un error inesperado!</strong><br>
                        El correo electronioc ingresado ya se encuentra registrado, por favor elija otro
                    </div>
                    ';
                    exit();
            }
    ...
    if ($usuario!=$datos['usuario_usuario']) {
        $check_usuario = conexion();
        $check_usuario = $check_usuario -> query("SELECT usuario_usuario FROM usario WHERE usuario_usuario='$usuario'");
    ...
    if ($clave_1!="" || $clave_2!="") {
        if (verificar_datos("[a-zA-Z0-9$@.-]{7,100}", $clave_1) || verificar_datos("[a-zA-Z0-9$@.-]{7,100}", $clave_2)) {
            ...
            if ($clave_1 != $clave_2) {
                ...
                <div class="notification is-danger is-light">
                    <strong>¡Ocurrio un error inesperado!</strong><br>
                    Las claves que ha agregado no coinciden
                </div>
                ';
                exit();
            }else {
    ...
    if ($actualizar_usuario -> execute($marcadores)) {
        ...
        echo '
            <div class="notification is-danger is-light">
                <strong>¡Ocurrio un error unesperado!</strong><br>
